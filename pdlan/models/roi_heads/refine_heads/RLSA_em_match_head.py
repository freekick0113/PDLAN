...
from pdlan.core import cal_similarity
from PIL import Image
import matplotlib.pyplot as plt
from ...mot.RLSA import RecurrentLinearSelfAttention
...
class RLSAEMMatchHeadPlus(nn.Module):
    def __init__(self,
                 num_convs=4,
                 in_channels=256,
                 conv_kernel_size=3,
        ...
        self.frame_num = 0
        self.momentum = 0.95
        self.att_refine = RecurrentLinearSelfAttention(in_channels)
        ...
        self.init_protos(pos_proto_num, 'pos_mu_key')
    ...
    def em_match(self, feat_a, mask_a, feat_b, mask_b):
        ...
        R = feat_b.size(0)
        pos_mu = pos_mu.permute(0, 2, 1).contiguous().view(R, -1, 1, 1) #R, K * C, 1, 1
        return pos_mu, pos_z, neg_z, pos_mu_c, neg_mu_c, pos_z_c, neg_z_c

    def use_RLSA(self, feat, mask, eps=1e-5):
        B, C, H, W = feat.size()

        fore_feat = (feat * mask).flatten(2).transpose(1, 2)                     # B * N * C

        if torch.cuda.is_available():
            fore_feat = fore_feat.to('cuda')
        x = feat * mask                                                          # B * C * H * W
        RLSA, _ = self.att_refine(x)                                             # B * N * C
        output = RLSA + fore_feat  # Add                                         # B * N * C
        fore_feat = F.layer_norm(output, output.size()[1:],eps=eps) #normalize
        fore_feat = fore_feat.transpose(1, 2).view(B, C, H, W)                   # B * C * H * W
        fore_feat_mu = self._em_iter(fore_feat, mask, self.pos_mu_key, 0)           # B * C * K
        with torch.no_grad():
            mu_p_key = fore_feat_mu.mean(dim=0, keepdim=True)
            self.pos_mu_key *= self.momentum
            self.pos_mu_key += mu_p_key * (1 - self.momentum)
        fore_sum = mask.view(B, 1, -1).sum(dim=2)                                #B * C
        fore_feat = fore_feat.view(B,C,-1).sum(dim=2)
        fore_feat_mu = fore_feat_mu.view(B,C,-1).sum(dim=2)
        fore_feat = fore_feat * 0.8 + fore_feat_mu * 0.2
        fore_feat = fore_feat / (fore_sum + eps)
        fore_feat = fore_feat.view(B, C, 1, 1)
        return fore_feat
    ...
    def forward(self, feat_a, mask_a, feat_b, mask_b):
        ...
        gap = self.use_RLSA(feat_a_ori, mask_a)
